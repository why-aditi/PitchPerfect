...
/**
 * Live event stream for one agent, carrying the PRD 6.2 envelope. EventSource reconnects
 * on its own, so there is no retry logic here.
 *
 * window.emit({type, session_id, ts, data}) still works from the console for driving the
 * UI with no backend running.
 */
export function subscribe(agentId: string, handler: Handler): () => void {
  const source = new EventSource(`/api/events?agent_id=${encodeURIComponent(agentId)}`);
