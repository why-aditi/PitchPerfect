...
3. ~~Should `escalate_to_human` use the engine's speak endpoint?~~ **Answered: yes.** Returning the hand-off line as text leaves it to the model to reword or bury, and the rep can arrive mid-sentence. The engine speaks it directly, with `interrupt: true` so it cuts through, fire-and-forget so a hand-off never blocks the reply. A session that never reached the engine simply skips it.
4. Does the Knowledge tab need CSV import for pricing tiers? Today it is a JSON textarea, which is enough for the demo but is the roughest edge an evaluator will touch. Keshav's call.
5. ~~Template or empty form for a new agent?~~ **Answered: template.** The console's editor starts from a blank `AgentConfig` carrying the seeded objection strategies, greeting, goal hierarchy and turn-detection defaults, so §15.2 only needs identity, tiers and one battlecard filled in.
